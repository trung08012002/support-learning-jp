...
const SideBar = () => {
    ...
    return (
        <aside className="flex-shrink-0 ">
            <nav className="h-full py-5 bg-white border-r  space-y-4" >
                {sideBarItems.map((sideBarItem, index) => <>
                    <h6 className="px-3 mb-2 pt-md-4" key={index}>{sideBarItem.header}</h6>
                    {
                        sideBarItem.menuItems.map((menuItem, index) => <SideBarItem key={index} Icon={menuItem.icon} href={menuItem.href} title={menuItem.title} />)
                    }
                </>)}
            </nav>
        </aside>
